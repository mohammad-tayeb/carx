...
export default function Page() {
    ...
    const handleSearch = async (e) => {
        ...
        setLoading(true)
        try {
            const res = await fetch(`/api/searchTestDriveAppointments?phone=${phone}`)
    ...
    return (
        <>
            <main className='flex justify-center items-center flex-col mt-10'>
                <h1 className='font-semibold mb-2'>Search Using Phone Number:</h1>
                ...
                {search && !loading && data.length === 0 && (
                    <h1 className="text-red-500 font-semibold">No data found</h1>
                )}
            </main>
        </>
    )
